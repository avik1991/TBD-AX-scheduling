...
The most ground-breaking technology taken from cellular networks is OFDMA which gives opportunity for the Access Point to serve several users simultaneously with frequency division. 
...
The main concern is just to be within the range of the 	
approachable Wi-Fi Access Point (AP). 
...
It allows the Wi-Fi Access Point to service several stations (STAs) simultaneously, to better cope with fading and, in case of uplink transmission, to improve the spectral power density. 
...
The resource management problem itself has been studied well in other OFDMA-systems, like cellular networks, for which this technology is not new. We can borrow some ideas from them as well as 11ax borrows OFDMA technology from cellular networks. We can implement 11ax Wi-Fi adaptation for known from mobile networks schedulers considering OFDMA features in Wi-Fi which impose some difficult constraints.  

In this paper, we study problems that arise while developing the schedulers for 11ax networks. The main goal of this paper is to introduce our method to allocate resources in 11ax networks. We also show how implement this method for some well-investigated schedulers. 
While we explored this method, this research incidentally pushed us to create a new type of resource schedulers, which we also offer to a reader's attention.
...
In Section~\ref{sec:scheduling}, we cover scheduling problem in 11ax networks and describe main principles of developed 11ax resource schedulers.
...
\todo{развернуто про мощность написать :(}
...
\todo{дописать + про информацию о мощности впилить, плюс упомянуть гораздо выше что это все аплинк плюс про BSR где-то скинуть. 367 страница стандарта и 67 тоже}
...
In \cite{lee-proportionalfair} authors try to adapt OFDMA schedulers in 3GPP LTE Uplink considering the contiguous allocation constraint appearing in those networks. They proved that scheduling problem with this condition is NP-Hard and offered some approximation algorithms to maximize proportional fair objective. However, when it comes to 11ax networks we should take into account additional condition that RU can consist only from special number of tones so we cannot unite RUs in arbitrary way as it can be done in 3GPP LTE. 

\todo{rewrite below}
In \cite{khorov2016several}, authors study the performance of the network
consisting of the legacy and 802.11ax STAs. Authors
propose an approach for optimal channel access parameter
values selection, which guarantees fairness between legacy
and 802.11ax and significantly increases the number of the
OFDMA transmissions for 802.11ax STAs. However, the
model developed in [2] allows only estimating the number
of the OFDMA transmissions in the network, but not the
achievable data rate. 

\todo{rewrite below}
Random access for UL OFDMA in 802.11ax has been
studied in \cite{lanante2017performance}. The authors consider a scenario, when STAs
...
methodology introduced in the paper might be used to
tune the random access for OFDMA UL, however, usage
of deterministic access is more feasible in terms of channel
efficiency for the studied scenario.

\todo{read and write below}
\cite{sharon2017scheduling}

\todo{ADD MATERIAL, oh, Idk articles and}
...
PF (Proportional Fair)  scheduler allocates RUs to the STAs in a way which maximizes 
\begin{equation}
\label{eq:11axpfutility}
U_{c,r}^{PF} = \sum_{i=1}^{n}\sum_{j=1}^{M} x_i^j \frac{r_i^j}{Q_i}
\end{equation}

in a time domain, where $Q_i$ is the average service rate, e.g. amount of data transmitted by the STA $i$ divided by its total time in system. This utility function is fully analogical to the objective function \eqref{eq:obj} with
\begin{equation}
\label{eq:pflambda}
...
It is obvious that this scheduler minimizes the upload time when conditions are met because the first STA finishes delivering its flow by time $\theta_1$ except for waiting.
...
So, this time would be minimal when we sort the STAs in the ascending order by $\theta_i$.

Actually, 11ax adaptation of this scheduler is not a trivial task. So in this subsection we design a novel family of 11ax schedulers called 11ax-SRTF or MUTAX, Minimizing Upload Time in 11ax networks based on classic SRTF.
While deriving formula we neglect the effects related to packetization (including aggregation and fragmentation) overhead. 

\todo{WHY I COPIED THIS mutax DESCRPITON??? pls rewrite harder paragraphs below, idk how, tbh, pls share with me}
...
At the second stage, we try to improve results from assumption that we are using only the whole channel by serving some flows in parallel for the nearest slot. So, we divide the channel into several RUs according to some RU~set.   

With defined notations in previous section, the total upload time $T\left(X\right)$ of existing flows differs from $T_{SRTF}$ in the following way. First, the upload time of each of $n$ flows increases by  the current slot duration $\tau$. Second, if $x_i^j=1$, the remaining amount of data of flow $i$ decreases by the amount of data the STA transmits in RU $j$ of the current slot: $\Delta D_i^j = \min\left\{D_i, \tau \times r_{i}^{j}\right\}$. Thus,  
$$
T\left(X\right) = n \tau + \sum_{i = 1}^{n} \left(n - i\right) \frac{D_i -  \sum_{k = 1}^{M} x_i^j \Delta D_i^j}{r_{i}}
$$

Since both $\tau$ and  $\Delta D_i^j$ depend on the resource allocation $X$, minimization of $T(X)$ requires exhaustive search by possible ways to allocate the RUs to the STAs like we offer in previous section, and to simplify the task we propose an heuristic approach based on two assumptions.
...
\todo{end of copypasted material}
...
\end{equation}
with help of which we can define the following optimization problem \eqref{eq:usl311}-\eqref{eq:usl314} with parameter lambda \eqref{eq:lambdasrtf}.

We can improve this approach to schedule current slot in assumption that afterwards it will work under classic the whole channel scheduler. Note that the AP has no need to reschedule resources with new TF unless new STA comes with new data to transmit or some of STAs end its transmission during the last TF slot. Thus, The AP can to leave the allocation of resources the same until the above conditions are met. 

But in this case we should change a bit previous optimization task. Now we have a new $n^* = n\tau\cdot\lceil\frac{\tau^*}{\tau}\rceil$, where $\tau^*$ is the smallest transmission length among all the STAs. We cannot obtain that parameter directly since it depends on the final RU set choice and its allocation, but we can make 1 step ahead and try to compute the smallest transmission time among all the STAs in the smallest RU. We also need to change subtracted part in numerator to satisfy recently changes $n$. So we get $D_i^{j*} = \min\left\{D_i, \tau\cdot\lceil\frac{\tau^*}{\tau}\rceil \times r_{i}^{j}\right\}$. Thus,  
\begin{equation}
\label{eq:timesrtfso}
T\left(X\right) = n^* \tau + \sum_{i = 1}^{n} \left(n - i\right) \frac{D_i -  \sum_{k = 1}^{M} x_i^j \Delta D_i^{j*}}{r_{i}}
\end{equation}
...
So analogically we get the same lambda for the \eqref{eq:usl311}-\eqref{eq:usl314} task.
\begin{equation}
\label{eq:lambdasrtfso}
\lambda_i^j = \left(n - i\right) \frac{\Delta D_i^{j*}}{r_{i}}
\end{equation}

Additionally, we can try to calculate desired time given by formula $\eqref{eq:timesrtfso}$ and search minimum among all RU sets. Thus, here we can define utility function:
\begin{multline}
