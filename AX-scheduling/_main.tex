...
In this paper, we study problems that arise while developing the schedulers for 11ax networks. The main goal of this paper is to introduce our method to allocate resources in 11ax networks. We also show how to implement this method for some well-investigated schedulers. 
...
Another problem is that to make it possible to use OFDMA all STAs should transmit with the same synchronized MCS. There is one exception in this rule: when we use 1024-QAM in big RU we still can transmit data in smaller ones with 256-QAM. This constraint comes from necessity to transmit data in a such way, that the AP gets the same receive power from all STAs. 
