...
\section{OFDMA features in 802.11ax}
\label{sec:features}
...
\begin{figure*}[t]
	\centering
		...
		\draw[draw = none,pattern=checkerboard light gray, pattern color=black] (8,1) rectangle (12,4.5);
		\draw [line width=0.5mm] (8, 1) rectangle (12, 4.5);
		\node [text width=3cm, align=center] at (10,  2.75) {EDCA TRANSMISSIONS};
...
Thus, OFDMA brings a lot of benefits to 11ax networks. In dense environments many users contend inefficiently with legacy methods for their turn to use the channel. 
...
In cellular networks PF algorithms usually consider each RU one-by-one, i.e. for RU $j$ it allocates the user with maximal~$r_i^j / Q_i$ at time slot $t$. But in a classic Wi-Fi there is only one RU representing the whole channel itself. So we assume that PF in legacy Wi-Fi works only with this RU.
...
Let slot be the time interval between two consequent TFs.
...
Note that the AP has no need to reschedule resources with new TF unless a new STA comes with new data to transmit or a STA ends its transmission during the last TF slot.
This idea is illustrated in Fig.~\ref{fig:mutexso}.
Thus, the AP can leave the allocation of resources the same until the above conditions are met. 
...
\begin{figure}[tb]
	...
	\begin{scaletikzpicturetowidth}{\columnwidth}
		...
		\draw [arrows={-triangle 45}] (0.5,1) -- (9.5,1);
		\draw [arrows={-triangle 45}] (0.5,1) -- (0.5,3.7);
		
		\node at (9.5,  1.3) {\textit{Time}};
		\node at (1.4,  3.5) {\textit{Frequency}};
		...
		\end{tikzpicture}
	\end{scaletikzpicturetowidth}
	\caption{\label{fig:SRPT} Workflow of the SRPT scheduler}
...
We use these coefficients to calculate the Hungarian algorithm and to find the best distribution of STAs among the RUs.
...
%In the scheduling algorithm, the AP, firstly, solves \eqref{eq:usl311}-\eqref{eq:usl314} task with \eqref{eq:lambdasrtfso} and then uses \eqref{eq:ulilitysrtfso} in the last condition part (line 5 of the pseudocode).
