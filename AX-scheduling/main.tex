...
\todo{%scenario: The network operates in a 40 MHz channel at 5 GHz.
  %Note that 
  %depending on allocation of RU set, 
  %MR allocates the whole channel to the STA which has the greatest transmission rate.
  only one resource unit - the whole channel itself.
  ...
  переписать этот раздел, ибо иногда сам читаю и кажется что списал с прошлой статьи
  }
...
%Here the second summand expresses the fact that STAs use the same transmission power to transmit in RUs regardless of their width. Thus, the resulting $SNR$ is higher for narrow RUs.
