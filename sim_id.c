...
//const double radius = 10; //10m; 37m;			 // 100 m
...
//const double AIFSN = 2;                 //2 2 2 3 7 --- dcf,vi,vo,be,bk
const double AIFS = SIFS + AIFSN * sigma;

//SIZE
//from 400 B => 100 KB => 1.5MB
const double MEAN = 14.28; //13.59 for 100 KB; 14.28 for 200 KB; 15.2 for 500 KB; 15.89 for 1MB; 18.20 for 10MB
const double FROM = 8 * 1000; //8*400//1e4;
const double TO =   8 * 3 * 1e6; //8*1.5*1e6//8e7;

//TIME ARRIVAL (TA)
const double TAFROM = 0.1;
...
struct STA {
	...
	int transmissions_per_flow;
        ...
        int big;		 	//large data or not?
        int satur;			//satur time or not?
};
struct Experiment {
	int index;
...
static int modula(double P, int rb) {
	if (P >= -46 && rb >= 9) return  11;		//1024-QAM 5/6
	if (P >= -46 && rb <  9) return  9;		//256-QAM 5/6
	if (P >= -48 && rb >= 9) return  10; 		//1024-QAM 3/4
	if (P >= -48 && rb <  9) return  9; 		//256-QAM 5/6
	if (P >= -51) return  9; 		//256-QAM 5/6
	if (P >= -53) return  8; 		//256-QAM 3/4
	if (P >= -58) return  7;		//64-QAM 5/6
	if (P >= -59) return  6;		//64-QAM 3/4
	if (P >= -60) return  5;		//64-QAM 2/3
	if (P >= -68) return  4;		//16-QAM 3/4
	if (P >= -71) return  3;		//16-QAM 1/2
	if (P >= -73) return  2;		//QPSK 1/2
	if (P >= -76) return  1;		//BPSK 1/2
	return 0;
}
static double tone26(int modul) {
	switch(modul) {
		...
		case 3: return 3.3;		//16-QAM 1/2
		case 4: return 5.0;		//16-QAM 3/4		
		case 5: return 6.7;		//64-QAM 2/3
		case 6: return 7.5;		//64-QAM 3/4
		case 7: return 8.3;		//64-QAM 5/6
		case 8: return 10.0;		//256-QAM 3/4
		case 9: return 11.1;		//256-QAM 5/6	
		case 10: return 11.1;		//1024-QAM 3/4 NO ^
		case 11: return 11.1; 		//1024-QAM 5/6 NO ^
...
static double tone52(int modul) {
	switch(modul) {
		...
		case 3: return 6.7;		//16-QAM 1/2
		case 4: return 10.0;		//16-QAM 3/4		
		case 5: return 13.3;		//64-QAM 2/3
		case 6: return 15.0;		//64-QAM 3/4
		case 7: return 16.7;		//64-QAM 5/6
		case 8: return 20.0;		//256-QAM 3/4
		case 9: return 22.2;		//256-QAM 5/6	
		case 10: return 22.2;		//1024-QAM 3/4 ^
		case 11: return 22.2; 		//1024-QAM 5/6 ^
...
static double tone106(int modul) {
	switch(modul) {
		...
		case 3: return 14.2;		//16-QAM 1/2
		case 4: return 21.3;		//16-QAM 3/4		
		case 5: return 28.3;		//64-QAM 2/3
		case 6: return 31.9;		//64-QAM 3/4
		case 7: return 35.4;		//64-QAM 5/6
		case 8: return 42.5;		//256-QAM 3/4
		case 9: return 47.2;		//256-QAM 5/6	
		case 10: return 47.2;		//1024-QAM 3/4 ^
		case 11: return 47.2; 		//1024-QAM 5/6 ^
...
static double tone242(int modul) {
	switch(modul) {
		...
		case 3: return 32.5;		//16-QAM 1/2
		case 4: return 48.8;		//16-QAM 3/4		
		case 5: return 65.0;		//64-QAM 2/3
		case 6: return 73.1;		//64-QAM 3/4
		case 7: return 81.3;		//64-QAM 5/6
		case 8: return 97.5;		//256-QAM 3/4
		case 9: return 108.3;		//256-QAM 5/6	
		case 10: return 121.9;		//1024-QAM 3/4
		case 11: return 135.4; 		//1024-QAM 5/6
...
static double tone484(int modul) {
	switch(modul) {
		...
		case 3: return 65.0;		//16-QAM 1/2
		case 4: return 97.5;		//16-QAM 3/4		
		case 5: return 130.0;		//64-QAM 2/3
		case 6: return 146.3;		//64-QAM 3/4
		case 7: return 162.5;		//64-QAM 5/6
		case 8: return 195.0;		//256-QAM 3/4
		case 9: return 216.7;		//256-QAM 5/6	
		case 10: return 243.8;		//1024-QAM 3/4
		case 11: return 270.8; 		//1024-QAM 5/6
...
static double tone996(int modul) {
	switch(modul) {
		...
		case 3: return 136.1;		//16-QAM 1/2
		case 4: return 204.2;		//16-QAM 3/4		
		case 5: return 272.2;		//64-QAM 2/3
		case 6: return 306.3;		//64-QAM 3/4
		case 7: return 340.3;		//64-QAM 5/6
		case 8: return 408.3;		//256-QAM 3/4
		case 9: return 453.7;		//256-QAM 5/6	
		case 10: return 510.4;		//1024-QAM 3/4
		case 11: return 567.1; 		//1024-QAM 5/6
...
static double truegensize(void) {
	return gensize(MEAN, FROM, TO);
}
...
static double truegenarrival(void) {
	return genarrival(TAMEAN, TAFROM, TATO);
}
static double max_mcs(double dist, int rb) {
	if(rb) {
		...
		double P = P0 - 10 * log10(T / TONES_IN_WHOLE_CHANNEL) - PL; //power
		printf("P = %lf\n", P);
...
static void transmit(struct Experiment *ex) {
	...
	for(struct STA *p = ex->stations; p - ex->stations != ex->nsta; p++) {
		if(p->da && p->left <= 0) {
			...
			p->start = ex->time + ex->slot_duration + truegenarrival();
			ex->delay += p->delay_aa;
...
static void maximize_metric(struct Experiment *ex) {
	...
	for (int rb_set = 0; rb_set < nda_sizes[index]; rb_set++) {
		...
		for(int mcs = min_mcs; mcs < 12; mcs++) {
